...
int main(int argc, char *argv[]){
    ...
    while(1){
    	...
    	//Used same socket and process structure as with other assignments.  
    	LABEL:
    	if (pid != 0){
    	...
    	//First recv loop is only looking for username and password data to log in. 
    	LABEL3:
    	if((read_size = recv(client_sock , client_message , 50 , 0)) > 0 ){
    		...
    		if(verified == 1){
    			...
    			//After log in is executed then user may now utilize the server.
    			LABEL2:
...
void get(char client_message[]){
	...
	//Parse through available files and check for needed file with sprintf.  
	while(tok != NULL){
		tok = strtok(NULL, "|");
		for(i = 1; i < 4; i++){
			bzero(filebuf, 500);
	...
	//After finding first file then look for second and send it. 
	strcpy(fileslist2, fileslist);
	tok = strtok(fileslist2, "|");
	while(tok != NULL){
		tok = strtok(NULL, "|");
...
void put(char client_message[]){
	...
	bzero(client_message, 500);
	strcpy(userstr, user);
	//Use the input from client to save the file and write in the data. 
	//Reads and writes are synced on size to prevent data overflow. 
	//All of the logic is on the client side so this is basic reading.  
...
void list(char client_message[]){
	...
	printf("called list\n");
	//Open directory and read contents into buffer with "|" for easy parsing on client side. 
	d = opendir(user);
	if(d == NULL){
...
int verify(char client_message[]){
	...
	//Open server config file and read into buffer for comparisions to the incoming
	//Client message with username and password.  
	...
	//Send back messages for any number of problems, user found but bad password, bad username, or logged in.  
